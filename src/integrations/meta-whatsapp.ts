...
export function verifyMetaWebhook(query: Record<string, unknown>) {
...
function extractMessages(body: any): MetaTextMessage[] {
  const entries = Array.isArray(body?.entry) ? body.entry : [];
  const all: MetaTextMessage[] = [];
  ...
  for (const entry of entries) {
    ...
    for (const change of changes) {
      ...
      for (const message of messages) {
        all.push(message as MetaTextMessage);
      }
    }
  }
...
export function normalizeMetaInbound(body: unknown): NormalizedInboundMessage[] {
  const messages = extractMessages(body as any);
...
export async function sendMetaTextMessage(params: { to: string; text: string }) {
