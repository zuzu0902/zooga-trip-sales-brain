  ...
  app.post('/webhooks/meta', async (request, reply) => {
    ...
    for (const inbound of inboundMessages) {
      ...
      await persistRuntimeWritebacks({
        phone: inbound.phone,
        contactId: result.trace && typeof result.trace === 'object' && typeof result.trace.runtimeLead === 'object' && result.trace.runtimeLead !== null && 'contactId' in (result.trace.runtimeLead as Record<string, unknown>)
          ? (((result.trace.runtimeLead as Record<string, unknown>).contactId as string | null) ?? null)
          : null,
        messageId: inbound.messageId,
        messageText: inbound.messageText,
