...
import { fetchLeadContextByPhone, generateReplyViaBridge, persistHandoffRequest, } from '../integrations/supabase-runtime-store.js';
...
function detectMode(messageText, offers, lastPresentedOffers) {
...
function buildGenericReply(name) {
...
function buildWritebacks(mode, leadState, offer, lastPresentedOffers) {
    const writebacks = [
        {
...
function buildObjective(mode, offer) {
    if (mode === 'browse')
        return { primary_goal: 'Present active trips naturally and preserve numbered selection continuity.', secondary_goal: 'Help the user continue with one specific option.' };
    if (mode === 'offer')
        return { primary_goal: `Confirm and sell the specific trip${offer ? `: ${offer.title}` : ''}.`, secondary_goal: 'Build confidence and move the user toward details or registration.' };
    if (mode === 'price')
        return { primary_goal: 'Answer the pricing question directly and honestly.', secondary_goal: 'Keep the user moving forward without unnecessary qualification.' };
    if (mode === 'handoff')
        return { primary_goal: 'Acknowledge the handoff calmly and honestly.', secondary_goal: 'Do not overpromise beyond the real delivery status.' };
...
function buildHardRules(mode, offer, allowLink) {
    const rules = [
        'Never invent facts.',
...
function buildMustInclude(mode, fallbackReply) {
    if (mode === 'handoff')
        return ['Be honest about the human follow-up status.', 'Do not exaggerate dispatch certainty.'];
    if (mode === 'price')
        return ['Answer the price question immediately in the first sentence if the price is known.'];
    if (mode === 'browse')
        return ['Mention actual active trip options, not a generic non-answer.', 'Preserve the meaning of the numbered list if present.'];
    return ['Stay consistent with the deterministic fallback reply intent.', fallbackReply];
}
function buildMustNotInclude() {
    return [
